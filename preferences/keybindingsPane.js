    ...
    class Combo extends GObject.Object {
        ...
        get keycode() {
            if (this.disabled) {
                ...
                let [ok, key, _mods] = this.acceleratorParse.accelerator_parse(
                    this.keystr
                );
...
const Keybinding = GObject.registerClass(
    ...
    class Keybinding extends GObject.Object {
        _init(settings, acceleratorParse, params = {}) {
            super._init(params);
            ...
            this._settings.connect(`changed::${this.action}`, () =>
                this._load()
            );
        ...
        replace(oldCombo, newCombo) {
            const [found] = this.find(newCombo);
            if (found) return;
        ...
        find(combo) {
            const pos = [...this.combos].findIndex(
                c => c.keystr === combo.keystr
            );
            if (pos === -1) {
                return [false];
            } else {
        ...
        _load() {
            ...
            let combos = keystrs
                ...
                .map(
                    ([, keyval, mods]) =>
                        new Combo(this.acceleratorParse, { keyval, mods })
                );
        ...
        _updateCollisions() {
            ...
            for (const binding of this._model) {
                for (const combo of binding.combos) {
                    ...
                    map.set(
                        combo.keystr,
                        (map.get(combo.keystr) || new Set()).add(binding.action)
                    );
                }
            }
            let changed = new Set();
...
const ComboRow = GObject.registerClass(
    ...
    class ComboRow extends Gtk.ListBoxRow {
        _init(params = {}) {
            ...
            this._conflictList.bind_model(this._collisions, binding =>
    ...
    class KeybindingsRow extends Gtk.ListBoxRow {
        _init(acceleratorParse, params = {}) {
            ...
            action.connect('activate', () =>
                this.keybinding.add(
                    new Combo(this.acceleratorParse, { placeholder: true })
                )
            );
            this._actionGroup.add_action(action);
...
export const KeybindingsPane = GObject.registerClass(
    ...
    class KeybindingsPane extends Gtk.Box {
        ...
        init(extension) {
            ...
            this._listbox.set_header_func((row, before, data) =>
                this._onSetHeader(row, before, data)
            );
        ...
        _createRow(keybinding) {
            const row = new KeybindingsRow(this.acceleratorParse, {
        ...
        _onSetHeader(row, before) {
            const header = row.get_header();
            if (
                !before ||
                before.keybinding.section !== row.keybinding.section
            ) {
